...
thunkify.runSelfAware = function(genFn, ...args) {
  let itGen = genFn(...args); // [2] Setup the function generator, but execute none of it.
  itGen.next(); // [3] Start the recursive "gen.next()" calls.
  let cbNext = clCbNext(itGen);
  cbNext(null, itGen); 
  return itGen; // Might also be useful (e.g. when dealing with nested thunks (see example3.js)).
};

thunkify.factory = function factory(genFn, isSelfAware = false) {
  if (isSelfAware){
    return function(...args) {
      return thunkify.runSelfAware(genFn, ...args);
    };
  }
  return function(...args) {
