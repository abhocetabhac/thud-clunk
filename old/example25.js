// Usage:
//   node ./example25 testfile2.txt testfile1.txt
//
// Nested thunks.
...
const fnRender = function fnRender(err = null, input = null) {
  if (err) {
    console.error(`Error: post-processing function "${fnRender.name}" could not render given "input".`);
  } else {
    console.log(input);
  }
  return input;
...
const fn1 = function(err = null, input = null) {
  if (err) {
    ...
    console.log('fn1:');
    if(input === 'Chatice dos infernos!\nPô!') {
      let exception = new Error('Erro na "Chatice dos infernos! Pô!"');
      exception.recoverable = true; // Signal an recoverable error to the generator function.
