    ...
    return function* (args) {
      ...
      try {
        ...
        for (; j < lenj; j++) {
          ...
          //console.log('io =', io)
          io = yield fns[j](null, io); // If an error is thrown from "fns[j]", it will be caught below and the "yield" will not occur.
        }
      } catch (err) { // Errors from the thunk are delegate by its callback to the generator.
        if (lenj === 0) 
          throw err;
        console.error('ERROR', j, lenj);
        console.log('Running error in', fns[j].name);
        let exception = fns[j](err, io); // Let the own "fns[j]" that thrown the error to treat it first.
        if (exception) { 
          for (j++; j < lenj; j++) { // Unrecoverable error
            console.error('Unrecoverable ERROR', j, lenj);
            console.log('Unrecover from error. Now in', fns[j].name);
            exception = fns[j](exception, io); // Unrecoverable error
          }
          return exception;    
        } else { // Recoverable error
          //io = yield io;// "fns[j] returned falsey, so it was considered a recoverable error.
          for (j++; j < lenj; j++) {
            console.error('ERROR recover', j, lenj);
            console.log('Recover from error. Now in', fns[j].name);
