...
const gnReadFile = function*(filepath, ...args) {
  let input;
...
thunkify.run = function run(genFn, ...args) {
  let gen = genFn(...args); // [2] Setup the function generator, but execute none of it.
  ...
  function cbNext(err = null, result = null) { 
    ...
    let more = gen.next(result); // [4] Execute the generator for the first time. "result" is ignored.
